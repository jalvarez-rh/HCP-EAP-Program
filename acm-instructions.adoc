...
In this enviroment you will noticed that you have 2 clusters already pre-build for you, 1 cluster named *development* is a Hosted Control Planes cluster hosted in AWS. You will also see a *local-cluster* this is your hub cluster just under management inside ACM

image::images/acm-images/acm-clusters.png[]

Feel Free to use this cluster as needed, you can also create aditional clusters, either traditional or HCP clusters.

== Create a new OpenShift Hosted Control Plane cluster in AWS
...
[start=5]
5. Steps 1 through 3 have been completed for you. Copy the command on Step number 4 

image::images/acm-images/hosted.png[]

[start=6]
6. Open a local terminal window and SSH to the Bastion host. Details for URL / login will be on the Workshop page on demo.redhat.com

image::images/acm-images/bastion.png[]

[start=7]
7. Once SSH, copy the login information for your Hub cluster. Paste the command on your Bastion Host and Login 

image::images/acm-images/login.png[]

[start=8]
8. Once logged in, navigate back to the OpenShift Console and Copy the information under the Step -4 / Create Cluster.
...
REGION="example"
CLUSTER_NAME="yourownclustername"
SECRET_CREDS="aws-credentials" 
NAMESPACE="example-namespace"  # $SECRET_CREDS needs to exist in $NAMESPACE.
INFRA_ID="example"
RELEASE_IMAGE="example"
...
[start=9]
9. Create a .sh script file on the Bastion Host VM with the following variables

----
REGION="us-west-2" # if you are outisde of the US use your local AWS region 
CLUSTER_NAME="yourownclustername"
SECRET_CREDS="aws-credentials"
NAMESPACE="local-cluster"
INFRA_ID="hosted"
RELEASE_IMAGE="quay.io/openshift-release-dev/ocp-release:4.13.12-x86_64"
----

[start=10]
10. Verify that you are logged in the Hub Cluster and execute the .sh file

image::images/acm-images/terminal.png[]

*One running this will take about 15-20 minutes depending on AWS current traffic*

[start=11]
11. Navigate back to the OpenShift Console and verify that the cluster is not sitting on *waiting to import* state.

*Please note that you might have to import this cluster manually even after is finished in order for ACM to manage it.*
